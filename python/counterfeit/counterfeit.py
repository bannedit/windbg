...
class counterfeit():
	def __init__(self):
		...
		self.hprocess = windll.kernel32.OpenProcess(PROCESS_ALL_ACCESS, False, self.pid)
	...
	def VirtualProtectEx(self, lpAddress, dwSize, flNewProtect, flOldProtect = 0):
		...
		'''
		self.size = dwSize
		flOldProtect = kernel32.VirtualProtectEx(lpAddress, dwSize, flNewProtect, flOldProtect)
		return flOldProtect
...
def main():
	...
	parser.add_argument("-a", "--allocate", metavar = "size", action = "store_true", help = "Allocate memory RWX")
	parser.add_argument("-p", "--protect", metavar = ("address", "size", "protection"), nargs = 3, action = "store_true", 
	...
	if args.allocate:
		addr = cf.VirtualAllocEx(0, int(size, 16), MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE)
		if args.fill:
